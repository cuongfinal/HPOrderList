...
class SSRadioButtonsController : NSObject
{
    ...
    */
    @objc func selectedButton() -> UIButton {
        guard let index = buttonsArray.index(where: { button in button.isSelected }) else { return buttonsArray.first! }
        
        return buttonsArray[index]
    }
    
    @objc func buttonIndex(_ aButton: UIButton) -> Int {
        if(buttonsArray.contains(aButton)){
            return buttonsArray.index(of: aButton)!
        }
        else{
            return 0
        }
